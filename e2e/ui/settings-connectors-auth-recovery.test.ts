...
async function waitForLoadingToClear(page: Page) {
  await expect(page.getByText('Loading Vision Design…')).toHaveCount(0, { timeout: 15_000 });
}
...
test.describe('Settings connectors auth recovery', () => {
  test('clears pending authorization when OAuth launch is blocked after redirect_required', async ({ page }) => {
    ...
    await expect(
      dialog.getByText('Popup blocked. Allow popups for Vision Design and try again.'),
    ).toBeVisible();
    await expect
      .poll(async () =>
