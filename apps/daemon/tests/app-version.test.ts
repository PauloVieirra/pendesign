...
describe('app version helpers', () => {
  ...
  it('prefers packaged app version metadata from the environment', () => {
    expect(resolveAppVersionInfo({
      ...
      resourcesPath: '/Applications/Vision Design.app/Contents/Resources',
      execPath: '/Applications/Vision Design.app/Contents/Resources/open-design/bin/node',
      platform: 'darwin',
      arch: 'arm64',
    })).toEqual({
  ...
  it('detects packaged runtimes without sidecar protocol knowledge', () => {
    expect(isPackagedRuntime({ resourcesPath: '/Applications/Vision Design.app/Contents/Resources' })).toBe(true);
    expect(isPackagedRuntime({
      execPath: '/Applications/Vision Design.app/Contents/Resources/open-design/bin/node',
      platform: 'darwin',
    })).toBe(true);
    expect(isPackagedRuntime({
      execPath: 'C:\\Users\\Ada\\AppData\\Local\\Programs\\Vision Design\\resources\\open-design\\bin\\node.exe',
      platform: 'win32',
    })).toBe(true);
    expect(isPackagedRuntime({
      execPath: '/opt/Vision Design/resources/open-design/bin/node',
      platform: 'linux',
    })).toBe(true);
