 ...
 *
 * Vision Design 0.3.x ran from the repo and wrote runtime state to
...
export function migrateLegacyDataDirSync(
  ...
  if (!legacyDirHasPayload(legacyDir)) {
    ...
    throw new LegacyMigrationError(
      'legacy_dir_invalid',
      `OD_LEGACY_DATA_DIR="${legacyDir}" is not a usable legacy data dir (expected app.sqlite directly inside it). Quit Vision Design, fix the path, and relaunch.`,
    );
  }
