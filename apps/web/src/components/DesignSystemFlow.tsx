        ...
        <button
          ...
          onClick={() => {
            if (!state.company.trim()) {
              setError('Tell Vision Design about the company or design system first.');
              return;
            }
            setStep('confirm');
          }}
        ...
        <section className="ds-resource-section">
          ...
          <div className="ds-resource-card">
            ...
            <DropZone
              ...
              helper="Vision Design can link a local folder for the agent to read, or copy a focused browser-selected folder snapshot into this design-system project."
  ...
  const sendProjectChatMessage = useCallback(
    async (
      ...
      if (config.mode !== 'daemon' || !config.agentId) {
        setChatError('Pick a local agent first, then ask Vision Design to update this design system.');
        return;
      }
          ...
          <div className="ds-review-column">
              ...
              <p>
                {generationActive
                  ? activeJob?.kind === 'revision'
                    ? 'Vision Design is applying your feedback. You can keep reviewing while the updated draft is prepared.'
                    : 'Vision Design is still working, but you can start giving feedback on the work so far.'
                  : 'Vision Design is ready for review. Give feedback on the work so far, then publish when it is useful for future projects.'}
              </p>
              <label>
                <input
                  type="checkbox"
            ...
            <div className="ds-warning-card">
              ...
              <span>
                ...
                Vision Design is rendering typography with substitute web fonts.
              </span>
    ...
    <section className={`ds-workspace-activity-card is-${status}`}>
      <div className="ds-workspace-activity-head">
        ...
        <span>
          <strong>
            {status === 'running'
              ? 'Vision Design is updating this system'
              : status === 'failed'
      ...
      <div>
        ...
        <span>
          <strong>
            {active
              ? job.kind === 'revision'
                ? 'Vision Design is revising'
                : 'Vision Design is still working'
              : job.status === 'failed'
...
function GitHubRepositoryAccessPanel({
  ...
  const methods: GitHubAccessMethod[] = [
    ...
    },
    {
      id: 'native-oauth',
      icon: 'link',
      title: 'Vision Design account',
      badge: 'Coming soon',
      tone: 'muted',
      description: 'Native GitHub sign-in managed by Vision Design; this build does not use an OD-managed GitHub token yet.',
    },
    {
      id: 'composio',
      icon: composioIcon,
    ...
    >
      <div className="ds-github-access-header">
        <span>
          ...
          <p>Paste a GitHub URL. Vision Design will use the first working access method.</p>
        </span>
        <button
          type="button"
...
function buildCreationAgentPrompt(
  ...
  const title = inferDesignSystemTitle(state);
  return [
    'Create this project as a complete Vision Design design system workspace.',
    '',
    ...
    state.codeFolders.length
      ? `Read the linked local code folders that Vision Design attached to this project: ${state.codeFolders.join(', ')}. Treat them as source context only unless the user asks you to edit them.\n\n${localFolderRunbook}`
      : '',
...
function buildSourceContextManifest(
  ...
  sections.push(
    ...
    '- README.md and SKILL.md should make the extracted system reusable as a real Vision Design design-system package.',
