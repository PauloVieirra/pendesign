...
export function effectivePreviewScale(
  ...
  if (viewport === 'web') return previewScale;
...
/**
 * Returns true when a source string will produce a visible preview.
 * Used by the Dual mode fallback: if the source is null or empty,
 * Dual degrades to Source (nothing to show in the right pane).
 */
...
function HtmlViewer({
  ...
  // When Edit mode activates, ensure every editable element in the current
  // file has a stable `data-od-id`. Without this, the bridge identifies
  // unannotated elements by a runtime DOM path that doesn't always survive
  // a source-side walk (host-node filtering, dynamic mutations, parser
  // differences) — and `applyManualEditPatch` returns "Target not found",
  // silently dropping the user's edit. The stamping is deterministic and
  // idempotent: subsequent activations on the same file are no-ops, and a
  // file regenerated by the agent gets re-stamped with the same ids in the
  // same document order.
  useEffect(() => {
    if (!manualEditMode) return;
    const base = sourceRef.current ?? source;
    if (base == null || base.length === 0) return;
    const signature = `${file.name}:${base.length}`;
    if (manualEditPrestampedRef.current === signature) return;

    const result = prestampHtmlForManualEdit(base);
    if (!result.changed) {
      manualEditPrestampedRef.current = signature;
      return;
    }
    ...
    void (async () => {
      try {
        const saved = await writeProjectTextFile(projectId, file.name, result.html, {
          artifactManifest: file.artifactManifest,
        });
        if (cancelled) return;
        if (saved) {
          sourceRef.current = result.html;
          setSource(result.html);
          setInlinedSource(null);
          // The iframe's `previewSource` is pinned to `manualEditFrozenSource`
          // while Edit is on so live style postMessages don't trigger reloads.
          // We MUST update the frozen copy too, otherwise the iframe keeps
          // showing the un-stamped HTML and the bridge keeps emitting
          // `path-*` ids for elements the source patcher can't find.
          setManualEditFrozenSource(result.html);
          manualEditPrestampedRef.current = `${file.name}:${result.html.length}`;
        }
      } catch {
        /* best-effort — the user may simply lose the binding persistence
           round if the disk write fails. They'll see "Target not found"
           on save which surfaces the issue. */
      }
    })();
    return () => { cancelled = true; };
  }, [manualEditMode, projectId, file.name, file.artifactManifest, source]);
  ...
  // Tracks `${file.name}:${source.length}` signatures we've already pre-stamped
  // so re-entering Edit mode doesn't repeatedly rewrite the same file. A new
  // signature (file changed on disk, agent regenerated, etc.) triggers a fresh
  // stamping pass.
  const manualEditPrestampedRef = useRef<string | null>(null);
  ...
  const useUrlLoadPreview = shouldUrlLoadHtmlPreview({
    mode: viewMode === 'source' ? 'source' : 'preview',
    ...
    // Dual mode edits the source via React state; the URL-load path serves
    // from disk and would not reflect unsaved edits, so force srcDoc so
    // CodeEditor commits propagate to the iframe in real time.
    forceInline: forceInline || needsSandboxShim || viewMode === 'dual',
  });
  ...
  useEffect(() => {
    if (!effectiveDeck || viewMode === 'source') return;
  ...
  function beginResponsiveResize(e: ReactMouseEvent) {
    e.preventDefault();
    ...
    const startW = Math.max(320, responsiveSize?.width ?? startRect.width);
    const startH = Math.max(400, responsiveSize?.height ?? startRect.height);
  ...
  // Width-only resize: dragging the right edge of the iframe wrapper.
  // Height stays at whatever it was (or container height when starting from null)
  // so the design behaves like Chrome DevTools' device mode — width tests
  // responsiveness without dragging the floor up and down.
  function beginResponsiveResizeWidth(e: ReactMouseEvent) {
    e.preventDefault();
    const wrapper = previewBodyRef.current;
    if (!wrapper) return;
    const startRect = wrapper.getBoundingClientRect();
    const startX = e.clientX;
    const startW = Math.max(320, responsiveSize?.width ?? startRect.width);
    const fixedH = Math.max(400, responsiveSize?.height ?? startRect.height);
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      setResponsiveSize({ width: w, height: fixedH });
    };
    const onUp = () => {
      document.body.style.cursor = '';
      window.removeEventListener('mousemove', onMove);
  ...
  // Effective view mode — Dual may be downgraded at runtime.
  // `viewMode` is the persisted intent; `effectiveViewMode` is what we actually render.
  // Manual-edit mode forces Preview so the inline ManualEditPanel has the full
  // canvas width; persisted Dual restores automatically once edit exits.
  const containerWidth = previewBodyRef.current?.clientWidth ?? 1024;
  const dualSupportsFile = isPreviewableFile(source);
  const dualFitsWindow = containerWidth >= 720;
  const dualAvailable = dualSupportsFile && dualFitsWindow;
  const effectiveViewMode: ViewMode = manualEditMode
    ? 'preview'
    : viewMode === 'dual' && !dualAvailable
      ? (dualSupportsFile ? 'preview' : 'source')
      : viewMode;

  const boardAvailable = effectiveViewMode === 'preview' && source !== null;
  const showPreviewToolbarControls = effectiveViewMode === 'preview';
    ...
    >
      ...
      >
        {previewViewport === 'responsive' ? (
          <>
            <div
              data-resize-handle-right="true"
              className="preview-resize-handle-right"
              onMouseDown={(e) => beginResponsiveResizeWidth(e)}
              onDoubleClick={() => setResponsiveSize(null)}
              aria-label="Resize design width"
              title="Drag to resize width · double-click to fit"
            />
            <div
              ...
              aria-label="Resize design width and height"
              title="Drag corner to resize both axes"
            />
          </>
        ) : null}
        {manualEditMode ? (
          ...
          <button
            ...
            onClick={() => {
              ...
              if (!manualEditMode) {
                ...
                setDrawOverlayOpen(false);
                setViewMode('preview');
                // Don't freeze the design width: the inspector column hadn't
                // opened yet when we'd capture it, so the frozen value would
                // be wider than the canvas column ends up being and the
                // design would overflow. Leaving null makes the shell use
                // 100% of the canvas column, which adapts to the panel.
                setManualEditViewportWidth(null);
