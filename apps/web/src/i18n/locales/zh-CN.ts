...
export const zhCN: Dict = {
  ...
  'settings.welcomeTitle': '初始化 Vision Design',
  ...
  'settings.agentInstall.pathHint':
    '如果你通过 npm 或 Homebrew 安装了 CLI，但仍显示为未安装，请确认该工具的 bin 目录已加入 Vision Design daemon 继承的 PATH（在 macOS 上，Terminal 与 GUI 应用的 PATH 可能不同）。请参阅 QUICKSTART.md（“Local agent CLI and PATH” 章节）。',
  'settings.agentInstall.stepOpenLinks': '在目标代理卡片上打开“安装”或“文档”链接。',
  'settings.agentInstall.stepAuth':
    '返回 Vision Design 之前，请先在对应 CLI 中完成认证（登录或添加 API 凭据）。',
  ...
  'settings.mcpServerHint': '将 Vision Design 作为 MCP 服务器暴露给你的编码代理。',
  ...
  'settings.privacy': '隐私',
  'settings.privacyHint': '与 Vision Design 团队共享哪些数据',
  'settings.privacyConsentKicker': '帮助我们改进 Vision Design',
  'settings.privacyConsentLead': 'Vision Design 可以将使用数据共享给我们的团队以协助改进。包括:',
  ...
  'settings.mcpHint':
    '让其他仓库中的编码助手（Claude Code、Cursor、VS Code、Antigravity、Zed、Windsurf）读取你的 Vision Design 项目。无需先导出 zip，即可将设计拉取到你的应用中。',
  'settings.mcpDaemonError':
    '无法连接到本地守护进程以解析安装路径（{error}）。请确保 Vision Design 正在运行，然后重新打开此面板。',
  ...
  'settings.mcpCapabilityDefault':
    '默认使用你在 Vision Design 中打开的项目和文件，因此你可以直接说"在我的应用中构建这个"，无需重复说明是哪个设计。',
  'settings.mcpRunningNote':
    'Vision Design 必须处于运行状态，MCP 工具调用才能成功。如果你在打开 Vision Design 之前启动了编码助手，请重启助手以便它能连接到正在运行的守护进程。',
  ...
  'homeHero.chip.createPluginHint': '创作可复用的 Vision Design 插件，并添加到我的插件。',
  ...
  'pluginsHome.subtitle': '当前运行环境内置的 Vision Design 工作流。选择一个加载 starter prompt，或浏览插件市场查看更多。',
  'pluginsHome.browseRegistry': '浏览插件市场',
  'pluginsHome.count': '{filtered} / {total}',
  'pluginsHome.loadingCatalog': '正在加载目录…',
  'pluginsHome.emptyCatalog': '目录为空。Vision Design 会随附内置插件，如果这里一直为空，请尝试重启 daemon。',
  ...
  'integrations.lede': '连接外部系统，把 MCP 工具带入智能体循环，并在其它 IDE、脚本和自动化中使用 Vision Design。',
  ...
  'mcpClient.daemonError': '无法连接本地 daemon。请确认 Vision Design 正在运行，然后重新打开此面板。',
  'mcpClient.saveFailed': '保存失败。请检查 daemon 是否正在运行后重试。',
  'useEverywhere.modalAria': '随处使用 Vision Design',
  'useEverywhere.modalTitle': '随处使用 Vision Design',
  'useEverywhere.modalSubtitle': '把 Vision Design 接入任何 IDE、智能体或脚本：CLI、HTTP、MCP 和 Skills。使用“复制智能体指南”，粘贴到 Claude Code、Codex、Cursor、openclaw 或 hermes 中即可完成配置。',
  ...
  'useEverywhere.section.overview.heading': 'Vision Design 可在智能体工作的任何地方运行',
  'useEverywhere.section.overview.intro': 'Vision Design 不只是一个窗口；它包含本地特权 daemon（`od`），以及 Skills + Design Systems + Atoms registry。它在你的机器上运行后，任何代码智能体（Claude Code、Codex、Cursor、OpenCode/openclaw、Hermes 或你自己的脚本）都可以通过四种可互换入口驱动生成、检查项目并产出设计制品。',
  'useEverywhere.section.overview.bullet1': 'CLI — 使用 `od <command>` 支持无头脚本、CI 和 shell 自动化。',
  'useEverywhere.section.overview.bullet2': 'MCP 服务器 — 将 Vision Design 作为 Model Context Protocol 服务器接入，使任何支持 MCP 的智能体都能列出技能、运行场景并读取制品。',
  'useEverywhere.section.overview.bullet3': 'HTTP API — `http://127.0.0.1:7456/api/*` REST + SSE 端点；与 Web UI 使用同一套接口。',
  'useEverywhere.section.overview.bullet4': 'Skills — 可直接放入的 `SKILL.md` 包（兼容 Claude），任何已在 PATH 中的智能体都能调用，甚至不需要打开 Vision Design。',
  ...
  'useEverywhere.section.cli.heading': '从任意 shell 驱动 Vision Design',
  ...
  'useEverywhere.section.mcp.heading': '把 Vision Design 暴露为任意代码智能体可用的 MCP 服务器',
  'useEverywhere.section.mcp.intro': 'Vision Design 内置 Model Context Protocol 服务器（`od mcp`），让任何支持 MCP 的客户端（Cursor、Claude Code、Antigravity、VS Code Copilot Chat、openclaw、hermes）都能发现 Vision Design 工具（列出技能、渲染预览、生成媒体、运行插件），无需手动 shell 调用。daemon 会通过 `GET /api/mcp/install-info` 为主流客户端发布可直接粘贴的安装片段。',
  ...
  'useEverywhere.section.mcp.footer': '在 Vision Design 应用中，打开“设置 → 集成”即可复制特定客户端的安装命令（Cursor、Claude Code、Antigravity、VS Code），无需手写 JSON。',
  ...
  'useEverywhere.section.skills.heading': '任意智能体可直接使用的 Skills，即使 Vision Design 未运行',
  'useEverywhere.section.skills.intro': 'Skill 是一个包含 Claude 兼容 `SKILL.md`（YAML front matter + 正文）的目录。Vision Design 通过 `od:` 命名空间（`mode`、`preview`、`design_system`、`inputs` 等）扩展该格式，使同一个制品既能在 Vision Design 内使用，也能被 Claude Code、Codex、openclaw 或 hermes 这类普通智能体使用。发现逻辑遵循优先级链，因此项目可以覆盖自己的 skills。',
  ...
  'useEverywhere.section.skills.snippet1': '最小 SKILL.md（Claude 兼容 front matter + Vision Design 扩展）',
  ...
  'entry.githubStarAria': '在 GitHub 上为 Vision Design 点亮 Star',
  ...
  'connectors.emptyNoMatchAction': '清除搜索',  'fileViewer.themeToggleToDark': 'Switch to dark mode',
  ...
  'ds.modal.search': '搜索',
  'ds.modal.filter': '筛选',
  'ds.modal.collections': '集合',
  'ds.modal.groups': '分组',
  'ds.modal.all': '全部',
  'ds.modal.createVariable': '创建变量',
  'ds.modal.createCollection': '创建集合',
  'ds.modal.createGroup': '创建分组',
  'ds.modal.addMode': '添加列',
  'ds.modal.renameMode': '重命名列',
  'ds.modal.deleteMode': '删除列',
  'ds.modal.deleteModeConfirm': '删除 {name} 列？该列中的值将丢失。',
  'ds.modal.empty': '暂无变量',
  'ds.modal.searchNoResults': '没有符合筛选条件的变量',
  'ds.modal.openShortcut': '打开设计系统',
  'ds.types.color': '颜色',
  'ds.types.number': '数字',
  'ds.types.string': '字符串',
  'ds.types.boolean': '布尔值',
  'ds.surfaceLabel': '类型',
  'ds.surfaceWeb': '网页',
  'ds.surfaceImage': '图片',
  'ds.surfaceVideo': '视频',
  ...
  'preview.errorBody': '示例 HTML 加载失败。请确认 Vision Design 正在运行后重试。',
  'preview.retry': '重试',
  ...
  'manualEdit.insert.text': "文本",
  'manualEdit.insert.shape': "形状",
  'manualEdit.insert.toolbarLabel': "插入元素",
  'manualEdit.size.widthLabel': "宽度",
  'manualEdit.size.heightLabel': "高度",
  'manualEdit.size.fixed': "固定",
  'manualEdit.size.fill': "填充",
  'manualEdit.size.hug': "自适应",
  'manualEdit.delete': "删除",
  'manualEdit.deleteConfirm.title': "删除此元素？",
  'manualEdit.deleteConfirm.body': "这将从设计中移除该元素。您可以从历史记录面板中撤销。",
  'manualEdit.deleteConfirm.cancel': "取消",
  'manualEdit.deleteConfirm.confirm': "删除",
  ...
  'fileViewer.codeEditor.title': '代码',
  'fileViewer.codeEditor.area': '代码编辑器',
  ...
  'pet.title': '宠物',
  'pet.tabBuiltIn': '内置',
  'pet.tabBuiltInHint': 'Vision Design 内置的精选宠物 — 一键领养。',
  ...
  'pet.codexBundled': '内置',
  'pet.codexBundledTitle': 'Vision Design 内置宠物，无需下载。',
  ...
  'settings.memoryStarterProjectDesc': 'Vision Design v0.5 — 聊天驱动的设计编辑器',
  ...
  'settings.pixabay.description': '图库照片搜索 — pixabay.com/api 免费密钥。每分钟 100 次请求。',
