...
afterEach(() => {
  ...
  // Phase 6 (dual view mode) persists viewMode to localStorage — clear it
  // between tests so mode selection in one test doesn't bleed into the next.
  try { localStorage.removeItem('od.fileViewer.viewMode'); } catch { /* ignore */ }
});
...
describe('FileViewer preview scale', () => {
  it('uses the requested zoom for web preview overlays', () => {
    expect(effectivePreviewScale('web', 1.5, { width: 320, height: 480 })).toBe(1.5);
  });
