...
function addBuildOptions(command: CacCommand, platform: ToolPackPlatform) {
  return command
    ...
    .option("--portable", "(default) build a redistributable artifact. Pass --no-portable to bake local tools-pack runtime roots into the packaged config — dev-loop testing only.")
