...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Vision Design-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Vision Design-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Vision Design"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Vision Design", "Vision Design.exe"),
    ...
    setupPath: join(namespaceRoot, "builder", "Vision Design-second-setup.exe"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Vision Design", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "Vision Design.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "Vision Design.exe"), "exe\n", "utf8");
      await writeFile(
      ...
      await expect(readFile(join(paths.unpackedRoot, "Vision Design.exe"), "utf8")).resolves.toBe("exe\n");
