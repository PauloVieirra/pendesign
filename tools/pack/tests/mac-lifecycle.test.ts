...
describe("startPackedMacApp", () => {
  it("writes a launch override when the bundled config is missing", async () => {
    ...
    try {
      ...
      const executablePath = join(paths.installedAppPath, "Contents", "MacOS", "Vision Design");
  ...
  it("uses the preview executable name for preview release namespaces", async () => {
    ...
    try {
      ...
      const paths = resolveMacPaths(config);
      const executablePath = join(paths.installedAppPath, "Contents", "MacOS", "Vision Design Preview");
